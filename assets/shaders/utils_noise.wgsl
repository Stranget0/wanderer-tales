#define_import_path wanderer_tales::utils_noise
...
fn value_noise_3d_dt(x: vec3<f32>) -> ValueDt3D {
    ...
    return ValueDt3D(k0 + k1 * u.x + k2 * u.y + k3 * u.z + k4 * u.x * u.y + k5 * u.y * u.z + k6 * u.z * u.x + k7 * u.x * u.y * u.z, du * vec3<f32>(k1 + k4 * u.y + k6 * u.z + k7 * u.y * u.z, k2 + k5 * u.z + k4 * u.x + k7 * u.z * u.x, k3 + k6 * u.x + k5 * u.y + k7 * u.x * u.y));
}
...
fn value_noise_2d(p: vec2f) -> vec3f {
    let i = floor(p);
    let f = fract(p);
